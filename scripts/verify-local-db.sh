...
read -r users spaces members clusters projects targets configs credentials releases commits runtime_targets execution_logs experiments audit_logs legacy_build_configs <<< "$counts"

if [[ "$users" != 1 || "$spaces" != 0 || "$members" != 0 || "$clusters" != 0 || "$projects" != 0 || "$targets" != 0 || "$configs" != 0 || "$credentials" != 0 || "$releases" != 0 || "$commits" != 0 || "$runtime_targets" != 0 || "$execution_logs" != 0 || "$experiments" != 0 || "$audit_logs" != 0 || "$legacy_build_configs" != 0 ]]; then
  printf 'Unexpected local TTP database counts:\n'
  printf 'users=%s spaces=%s members=%s clusters=%s projects=%s targets=%s configs=%s credentials=%s releases=%s commits=%s runtime_targets=%s execution_logs=%s experiments=%s audit_logs=%s legacy_build_configs=%s\n' \
	    "$users" "$spaces" "$members" "$clusters" "$projects" "$targets" "$configs" "$credentials" "$releases" "$commits" "$runtime_targets" "$execution_logs" "$experiments" "$audit_logs" "$legacy_build_configs"
  exit 1
fi
