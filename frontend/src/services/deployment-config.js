...
export function parseManifest(manifest) {
  ...
  return expandDocuments(values)
}

export function formatManifest(manifest, format) {
  const resources = parseManifest(manifest)
  if (format === 'json') {
    return `${JSON.stringify(resources.length === 1 ? resources[0] : resources, null, 2)}\n`
  }
  return resources.map((resource) => stringify(resource, { indent: 2 })).join('---\n')
}
...
function expandDocuments(values) {
