...
后端和前端启动后，使用默认管理员执行只读检查。空库初始化后的登录账号固定为 `admin`，密码固定为 `ttp`。脚本会验证健康接口、匿名访问、错误密码、登录会话、空间为空和未选择空间时的隔离行为，不会写数据库：
...
需要验证项目、环境、Manifest、真实 Git 分支和发布单的生命周期时，显式开启可清理的变更检查：

```bash
RUN_MUTATING_API_CHECK=YES scripts/api-regression.sh
```

该模式只删除脚本创建的临时空间，保留正在运行的默认管理员和其他既有业务数据。真实 Git 读取受仓库可见性、Token 和网络影响；镜像构建、真实发布和非滚动流量切换没有配置时，脚本会把外部依赖失败报告出来，不伪造成功。
...
- 正式发布必须配置真实 `ttp-builder`。项目只需在仓库根目录提供 `Dockerfile`；Builder 的推送凭证引用和构建平台由平台运维环境配置。项目还可在"项目设置"里填写自己的镜像仓库地址（`registry.example.com/命名空间/仓库`，不带 tag）：发布产物优先推送到该仓库，其 registry 主机必须匹配 builder 凭证映射中的某一条，否则发布会在构建阶段明确失败；留空则继续使用平台默认的 `前缀/project-<项目哈希>` 命名。没有 Builder 时，平台会明确返回 `image_build_unsupported`，不会把 commit SHA 冒充镜像，也不会报告 Kubernetes 发布成功。
